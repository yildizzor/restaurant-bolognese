...
router.post("/order/item/:id", isLoggedIn, async (req, res, next) => {
  const { id } = req.params;
...
router.post("/order/item/remove/:id", isLoggedIn, async (req, res, next) => {
  const { id } = req.params;
  const { currentPage } = req.body;
  try {
...
router.post("/order/item/delete/:id", isLoggedIn, async (req, res, next) => {
  const { id } = req.params;
  const { currentPage } = req.body;
  try {
    const currentUserId = req.session.currentUser._id;
...
router.post("/order/cancel/:id", isLoggedIn, async (req, res, next) => {
